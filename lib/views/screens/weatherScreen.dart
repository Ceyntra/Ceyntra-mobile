...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  Future<String> getWeatherData() async {
    ...
    http.Response response = await http.get(Uri.parse(
        'https://api.openweathermap.org/data/2.5/onecall?lat=$lat&lon=$lon&exclude=hourly,minutely,alerts&appid=b6888446a38baf7b596417ccd83ce056'));
    ...
    if (response.statusCode == 200) {
      ...
      await setState(() {
        weatherModel = new WeatherModel(
          //dateObject, place, country, condition, tempeture, windStatus, visibility, humidity, airPressure, weatherForecast
...
class WeatherContent extends StatelessWidget {
  ...
  TextStyle fontStyleCircle = GoogleFonts.montserrat(
      color: Colors.black, fontSize: 64, fontWeight: FontWeight.w300);
          ...
          Expanded(
            ...
            child: Container(
              child: Column(
                children: [
                  Text(
                    'Today, ${DateFormat('MMM').format(weatherModel.dateObject)} ${weatherModel.dateObject.day}th ${weatherModel.dateObject.year}',
                    style: GoogleFonts.montserrat(
                        color: Colors.white,
                        fontSize: 12,
            ...
            child: Container(
              child: Row(
                ...
                children: [
                  WeatherDetails(
                    title: 'Wind status',
                    data: weatherModel.windStatus,
                    dataUnit: 'mph',
                  ),
                  WeatherDetails(
                    title: 'Visibility',
                    data: weatherModel.visibility / 1000,
                    dataUnit: 'miles',
                  ),
                ],
              ),
                  ...
                  Row(
                    children: [
                      for (int i = 0; i < 5; i++)
                        Expanded(
                          flex: 1,
                          child: NextDayWeather(
                            dateObj: weatherModel.dateObject,
                            dayIndex: i + 1,
                            weatherForecast: weatherModel.weatherForecast,
                            weatherIcon: weatherModel.getWeatherIcon(
                                weatherModel.weatherForecast[i]['weather'][0]
                                    ['id']),
                          ),
                        ),
                    ],
